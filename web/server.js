...
const processNextInQueue = async () => {
  ...
  if (waitingQueue.length > 0 && availableBrowserslot) {
    ...
    try {
      ...
      nextRequest.res
        .status(500)
        .send({ error: "Failed to allocate browser." });
    }
...
setInterval(async () => {
  ...
  for (const [browserslot, browserEntry] of Object.entries(browserPool)) {
    if (
      browserEntry.status === "not" &&
      now - browserEntry.lastActivity > 600000
    ) {
...
function findPageByPageId(browserId, pageId) {
  const slot = Object.keys(browserPool).find(
    (slot) => browserPool[slot].browserId === browserId
  );
...
app.post("/closeBrowser", async (req, res) => {
  ...
  if (!browserId) {
    return res
      .status(400)
      .send({ error: "Missing required field: browserId." });
  }
...
function parseAccessibilityTree(nodes) {
  ...
  const IGNORED_ACTREE_ROLES = ["gridcell"];
  ...
  function dfs(idx, depth, parent_name) {
    ...
    try {
      ...
      if (
        !name.trim() ||
        IGNORED_ACTREE_ROLES.includes(role) ||
        (parent_name.trim().includes(name.trim()) &&
          ["StaticText", "heading", "image", "generic"].includes(role))
      ) {
        validNode = false;
      } else {
        let properties = [];
      ...
      if (validNode) {
        ...
        treeStr += `${indent}[${
          Object.keys(treeIdxtoElement).length
        }] ${nodeStr}`;
      }
    } catch (e) {
      validNode = false;
...
async function getBoundingClientRect(client, backendNodeId) {
  try {
    ...
    const remoteObject = await client.send("DOM.resolveNode", {
      backendNodeId: parseInt(backendNodeId),
    });
  ...
  if (accessibilityTree[parentCursor] !== undefined) {
    ...
    childrenNodeids.forEach((childNodeid, idx) => {
      if (childNodeid in nodeidToCursor) {
        accessibilityTree[parentCursor].childIds.splice(
          index + idx,
          0,
          childNodeid
        );
...
function getInViewportRatio(
  ...
  const overlapWidth = Math.max(
    0,
    Math.min(elemRightBound, winRightBound) -
      Math.max(elemLeftBound, winLeftBound)
  );
  const overlapHeight = Math.max(
    0,
    Math.min(elemLowerBound, winLowerBound) -
      Math.max(elemTopBound, winTopBound)
  );
...
app.post("/getAccessibilityTree", async (req, res) => {
  ...
  try {
    ...
    const [axtree, backendDOMids] = await fetchPageAccessibilityTree(
      response.nodes
    );
    console.log("finished fetching page accessibility tree");
    const boundingClientRects = await fetchAllBoundingClientRects(
      client,
      backendDOMids
    );
    ...
    if (pageEntry["downloadedFiles"].length > 0) {
      if (
        pageEntry["downloadSources"].length <
        pageEntry["downloadedFiles"].length
      ) {
      ...
      downloadedFiles.forEach((file, idx) => {
        yamlWithPrefix += `File ${idx + 1} (from ${
          pageEntry["downloadSources"][idx]
        }): ${file}\n`;
      });
    }
...
async function clickElement(click_locator, adjust_aria_label, x1, x2, y1, y2) {
  const elements = adjust_aria_label
    ? await click_locator.elementHandles()
    : await click_locator.all();
...
app.post("/performAction", async (req, res) => {
  ...
  if (
    ["click", "type"].includes(actionName) &&
    (!browserId || !actionName || !targetElementType || !pageId)
  ) {
    ...
    switch (actionName) {
      case "click":
        ...
        if (!clicked) {
          ...
          if (!clicked) {
            const targetElementNameStartWords = targetElementName
              .split(" ")
              .slice(0, 3)
              .join(" ");
            const click_locator = await page.getByText(
              targetElementNameStartWords
            );
            clicked = await clickElement(
              click_locator,
              adjust_aria_label,
            ...
            if (!clicked) {
              return res
                .status(400)
                .send({ error: "No clickable element found." });
            }
          }
        }
      ...
      case "type":
        ...
        try {
          locator = await page
            .getByRole(targetElementType, {
              name: targetElementName,
              exact: true,
              timeout: 5000,
            })
            .first();
          type_clicked = await clickElement(
            locator,
            adjust_aria_label,
            node.union_bound.x,
            node.union_bound.x + node.union_bound.width,
            node.union_bound.y,
            node.union_bound.y + node.union_bound.height
          );
        } catch (e) {
          console.log(e);
          type_clicked = false;
        ...
        if (!type_clicked) {
          locator = await page
            .getByRole(targetElementType, { name: targetElementName })
            .first();
          type_clicked = await clickElement(
            locator,
            adjust_aria_label,
        ...
        break;
      case "select":
        let menu_locator = await page.getByRole(targetElementType, {
          name: targetElementName,
          exact: true,
          timeout: 5000,
        });
      ...
      case "scroll":
        if (actionValue === "down") {
          ...
          return res
            .status(400)
            .send({ error: "Unsupported scroll direction." });
        }
        break;
      case "goback":
        await page.goBack();
...
app.post("/takeScreenshot", async (req, res) => {
  ...
  if (!browserId || !pageId) {
    return res
      .status(400)
      .send({ error: "Missing required fields: browserId, pageId." });
  }
